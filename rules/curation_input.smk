...
localrules: draft_assembly_polished_scaffolds, generate_yaml_polished
...
rule draft_assembly_polished_scaffolds:
    ...
    params:
        ...
        out_htigs="{species}/assembly/draft/{sample}.PB.asm1.purge1.polish1.scaff1/{sample}.PB.asm1.purge1.polish1.scaff1.htigs.fa.gz",
    ...
    shell:
        ...
        "[ -f {params.in_mito} ] && bgzip -c {params.in_mito} > {params.out_mito}"
...
rule generate_yaml_polished:
    ...
    params:
        ...
        haplotigs=asm_dir+"{species}/assembly/draft/{sample}.PB.asm1.purge1.polish1.scaff1/{sample}.PB.asm1.purge1.polish1.scaff1.htigs.fa.gz",
...
rule align_hic_mq0_polished_scaffolds:
    input:
        scaff="{species}/working/{sample}.{assembler}.{date}/scaff_polished.{purge_dir}.hic.{hic_sample}/out.break.salsa2/scaffolds_FINAL.fasta",
        scaff_index="{species}/working/{sample}.{assembler}.{date}/scaff_polished.{purge_dir}.hic.{hic_sample}/out.break.salsa2/scaffolds_FINAL.fasta.bwt",
        hic_fofn="{species}/working/{sample}.{assembler}.{date}/wdl-{purge_dir}/{hic_sample}.hic-arima2.fofn"
    output:
        "{species}/working/{sample}.{assembler}.{date}/scaff_polished.{purge_dir}.hic.{hic_sample}/out.break.salsa2/alignment_mq0/aligned.bam"
    params:
        queue="long"
    conda: "bwa.yml"
    ...
    shell:
        ...
        "samtools bam2fq -@ {threads} -0 /dev/null $cram | bwa mem -t {threads} -p {input.scaff} - > {output}"
